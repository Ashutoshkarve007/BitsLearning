...
START
        ADR R4, SRC            ; Load the address of the BITS ID (01021)
        LDR R5, [R4]           ; Load the value (01021) into R5
        ADR R6, DST            ; Load the address of DST to store results
        BL SUB1                ; Call the SUB1 function to count 0's and 1's
STOP    
        B STOP                 ; Infinite loop to stop the program

SUB1    
        MOV R0, #0             ; Initialize R0 to store the count of 0's
...
DST     DCD 0,0                ; Define two words to store the count of 0's (DST[0]) and 1's (DST[1])
