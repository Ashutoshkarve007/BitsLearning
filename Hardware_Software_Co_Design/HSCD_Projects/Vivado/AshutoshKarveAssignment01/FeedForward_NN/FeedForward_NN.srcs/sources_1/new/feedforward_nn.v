...
// 
// Create Date: 10/10/2024 08:19:49 AM
...
module feedforward_nn(
    ...
    // Weight and bias registers for hidden and output layers
    reg signed [7:0] w_hidden[0:2][0:3]; // 3 hidden neurons, each with 4 inputs
    reg signed [7:0] b_hidden[0:2];      // Biases for hidden layer neurons

    reg signed [7:0] w_output[0:1][0:2]; // 2 output neurons, each with 3 inputs (from hidden layer)
    reg signed [7:0] b_output[0:1];      // Biases for output layer neurons

    // Internal signals for hidden layer outputs
    reg signed [7:0] hidden_output[0:2]; // Outputs from hidden layer neurons
    ...
    always @(*) begin
        out1 = hidden_output[0] * w_output[0][0] + hidden_output[1] * w_output[0][1] + hidden_output[2] * w_output[0][2] + b_output[0];
        out2 = hidden_output[0] * w_output[1][0] + hidden_output[1] * w_output[1][1] + hidden_output[2] * w_output[1][2] + b_output[1];
    end
